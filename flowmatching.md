...
| $g(t)$ | **扩散系数**：单位时间内"随机抖动"的强度（量纲 = 位置/√时间）；我们**自己选**，SmolVLA 取 $g(t)=\eta\sqrt t$（$\eta$ 是常数噪声强度）| `diffusion_scale` |
| $p_t(x)$ | 边缘分布：所有粒子在时刻 $t$ 的"密度形状" | 不用显式算 |
| $s(x_t,t)$ | score：$s=\nabla_x\log p_t(x)$——密度"增长最快的方向" | 后面推出的闭式 |
| $dW$ | **布朗运动的无穷小增量**：$dW \sim \mathcal{N}(0, dt)$（标准差 $\sqrt{dt}$）；它的"离散版"是 $\sqrt h\cdot\xi$（$\xi\sim\mathcal{N}(0,1)$）| 采样噪声 |
| $\xi$ | 一个标准正态随机数（$\xi\sim\mathcal{N}(0,1)$），每次采样独立 | 采样噪声 |
| $h$ | 去噪步长（$h=1/\text{num\_steps}$）| `step_size` |

### 2. 方法论（一步步，每一步说明"为什么走到下一步"）

**第 1 步：先学会"读"随机微分方程（SDE）——重点讲清 $dW$ 和 $g$ 到底是什么。**

一个 SDE 写成 $dx = f(x,t)\,dt + g(t)\,dW$，它说的是"粒子怎么随时间动"，由两股力合成：**漂移 $f\,dt$（确定性的推）+ 扩散 $g\,dW$（随机性的抖）**。下面把"抖"这半句拆到骨头里。

**① $dW$ 是什么——从"随机游走"讲起。**

想象一个醉汉每秒钟向前或向后走一步（各 50% 概率，步长 1）。这是离散的"随机游走"。**布朗运动 $W(t)$ 就是把这个随机游走的"步长→0、每秒步数→∞"的连续极限**，它是 $dW$ 的"积分版"。对我们来说只需要它一个性质：

$$W(t+dt) - W(t) \;\sim\; \mathcal{N}(0,\ dt)\qquad\text{（增量是零均值、方差 $dt$ 的高斯）}$$

即"布朗运动每走一小段时间 $dt$，位置的**增量**是一个标准差为 $\sqrt{dt}$ 的随机数"。$dW$ 就是这个无穷小增量本身。

**② 为什么增量标准差是 $\sqrt{dt}$ 而不是 $dt$（这是全篇最容易晕的点）。**

原因：随机游走的**位移平方**可以叠加（方差可加）。数一下：若每步的方差是 $\sigma^2$，走 $N$ 步后的总位移方差 $=N\sigma^2$。换成连续时间：把时间 $T$ 切成 $N=T/dt$ 段，每段增量方差 $=dt$，则 $T$ 时刻总位移方差 $=N\cdot dt=(T/dt)\cdot dt=T$——**与时间成正比，所以总位移标准差 $=\sqrt{T}$**（这就是物理里著名的"扩散距离 $\propto\sqrt t$"定律）。

> **数值例子（理解 $\sqrt{\ }$ 的关键）**：
> - 随机游走走 $N=100$ 步（每步 $\pm1$），最终位置标准差 $=\sqrt{100}=10$，不是 $100$——步子互相抵消，不会越走越远到线性量级；
> - 走 $N=10000$ 步，标准差 $=\sqrt{10000}=100$，只是 10 倍（步数 100 倍）；
> - 所以**每单步必须用 $\sqrt{dt}$ 量级**：若单步用 $dt$（比 $\sqrt{dt}$ 小得多），$N$ 步累计方差 $=N\cdot dt^2=(T/dt)dt^2=T\cdot dt\to0$——时间切得越细扩散越消失，这就不叫扩散了。只有 $\sqrt{dt}$ 量级才能让"扩散速率"与怎么切时间无关。

**③ $dW$ 的记号习惯（不吓人版）**：布朗运动处处连续但处处不可导，所以 $dW$ **不能像 $dt$ 那样"除以 $dt$"当普通导数用**；它只以"增量/积分"形式出现。我们也不需要它的光滑性质，只需要：**离散化时把 $dW$ 替换成 $\sqrt h\cdot\xi$**（$h$ 是步长、$\xi\sim\mathcal{N}(0,1)$ 每次独立采样）——这是全程唯一会用到的翻译。

**④ $g(t)$ 是什么——"每秒抖多快"的旋钮。**

- $g(t)$ 叫**扩散系数**：它决定"单位时间里随机抖动的强度"。整项 $g(t)\,dW$ 的**单步量级 = $g\cdot\sqrt{dt}$**（$g$ 乘上 $\sqrt{dt}$ 的标准差）；
- $g$ 可以是常数，也可以**依赖时间**（写作 $g(t)$）甚至依赖位置（一般 SDE 的 $g(x,t)$；本文只需 $g(t)$）；
- 物理含义：$g$ 越大，粒子越"毛躁"、单位时间被抖散得越远；$g=0$ 就没有随机性（退化成 ODE）；
- **SmolVLA 取 $g(t)=\eta\sqrt t$**：$\eta$ 是常数（本项目 0.05），$\sqrt t$ 让噪声**随时间衰减**——$t=1$（噪声端）时 $g=\eta$ 最大，$t\to0$（接近动作）时 $g\to0$，保证最后几步几乎不抖、动作质量不毁。

> **$g\,dW$ 的"单步 vs 累计"数字例**（把 ② 和 ④ 合起来）：取 $g=0.2$、步长 $h=0.25$（第 5 章样例的参数）——
> - **单步**抖动量级 = $g\sqrt h = 0.2\times\sqrt{0.25} = 0.1$（这就是转移标准差 $\sigma$）；
> - 走 4 步（总时间 $T=1$）后的累计位移标准差 $=g\sqrt T=0.2\times1=0.2$——因为 4 步的方差 $=4\times0.1^2=0.04$，开根 $=0.2$ ✓；
> - 如果错误地用 $g\cdot h=0.05$ 当单步，4 步累计 $=\sqrt{4\times0.05^2}=0.1$，只有正确值的一半——**再次印证单步必须是 $g\sqrt h$（即 $dW$ 的 $\sqrt{dt}$ 量级）。**

> **离散化速记**（后面所有推导只用这条）：$dt \to h$（步长），$dW \to \sqrt h\,\xi$（$\xi\sim\mathcal{N}(0,1)$），所以 $g\,dW \to g\sqrt h\,\xi$，SDE 的离散形式就是 $x_{t+h}=x_t+f\cdot h+g\sqrt h\,\xi$。
